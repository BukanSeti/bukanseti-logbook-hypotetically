...
def test_manual_review_requests_icao_aircraft_and_preserves_crew_privacy(
    tmp_path: Path,
):
    ...
    assert document["aircraft_questions"][0]["rerun_example"] == (
        "--aircraft PK-LJF=B739"
    )
    assert document["aircraft_questions"][0]["missing"] == [
        "aircraft_type_icao"
    ]
    assert "ICAO designators only" in document["aircraft_type_standard"]
    assert "four-letter ICAO" in document["airport_code_standard"]
